...
(define (domain basic-viajes)
  ...
  (:action siguiente-ciudad
    ...
    :precondition (and (in ?ciutat1) (not-visited ?ciutat2) (transporte ?ciutat1 ?ciutat2) (localizado ?ciutat2 ?htl)  
                  (>= (dias_city) (min_days_city)) (<= (dias_city) (max_days_city)))
    :effect (and (not (in ?ciutat1)) (in ?ciutat2) (visited ?ciutat2) (not (not-visited ?ciutat2)) (hospedado ?ciutat2 ?htl) 
            (increase (min_total_dias_rec) 1) (at start(decrease (aux_data) 1)) (at end(decrease (dias_city) (aux_data)))))

  (:action misma-ciudad
    :parameters (?ciutat1 - ciudad ?htl - hotel)
    :precondition (and (in ?ciutat1) (localizado ?ciutat1 ?htl) (not (starting ?ciutat1)))
    :effect (and  (increase (min_total_dias_rec) 1) (increase (dias_city) 1) (increase (aux_data) 1)))
